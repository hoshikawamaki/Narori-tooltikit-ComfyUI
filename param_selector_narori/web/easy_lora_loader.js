...
app.registerExtension({
    name: "Narori.EasyLoraLoader",
    
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "NaroriEasyLoraLoader") {
            const onNodeCreated = nodeType.prototype.onNodeCreated;
            
            nodeType.prototype.onNodeCreated = function() {
                const result = onNodeCreated?.apply(this, arguments);
                
                // 创建隐藏的widget来存储提示词数据（每个节点实例独立保存）
                const promptsWidget = this.addWidget("text", "prompts_data", JSON.stringify({}), () => {}, {serialize: true});
                ...
                // 添加自定义UI行（在lora下拉框之后显示）
                this.addCustomWidget({
                    name: "lora_prompt_selector_row",
                ...
                this.showConfigDialog = (promptsWidget, indexWidget) => {
                    ...
                    dialog.innerHTML = `
                        <h3 style="margin:0 0 15px 0;color:#fff;font-size:18px;">📝 Lora提示词配置管理</h3>
                    ...
                    dialog.querySelector("#addRowBtn").onclick = () => {
                        const existingCount = Object.keys(prompts).length;
                        const newName = `Lora提示词${existingCount + 1}`;
                        prompts[newName] = "";
                        renderTable();
                    };
